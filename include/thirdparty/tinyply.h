...
struct PlyFile {
     ...
     * runtime which properties or elements exist in the file. Limited
     * validation of the header is performed; it is assumed the header correctly
     * reflects the contents of the payload. This function may throw. Returns
    ...
    /*
     * These functions are valid after a call to `parse_header(...)`. In the
     * case of writing, get_comments() may also be used to add new comments to
     * the ply header.
     */
...
struct PlyFile::PlyFileImpl {
    ...
    size_t read_property_binary(const Type& t,
                                const size_t& stride,
                                void* dest,
                                size_t& destOffset,
                                std::istream& is);
    ...
    std::vector<std::vector<PropertyLookup>> make_property_lookup_table() {
        ...
        for (auto& element : elements) {
            ...
            for (auto& property : element.properties) {
                ...
                auto cursorIt =
                    userData.find(hash_fnv1a(element.name + property.name));
    ...
    void write_property_ascii(Type t,
                              ...
                              uint8_t* src,
                              size_t& srcOffset);
    void write_property_binary(Type t,
                               std::ostream& os,
                               uint8_t* src,
                               size_t& srcOffset,
                               const size_t& stride);
...
template <typename T, typename T2>
inline void endian_swap_buffer(uint8_t* data_ptr,
...
void PlyFile::PlyFileImpl::write_property_ascii(Type t,
                                                std::ostream& os,
                                                uint8_t* src,
                                                size_t& srcOffset) {
    switch (t) {
...
void PlyFile::PlyFileImpl::write_property_binary(Type t,
                                                 std::ostream& os,
                                                 uint8_t* src,
                                                 size_t& srcOffset,
                                                 const size_t& stride) {
        ...
        for (auto& entry : userData) {
            if (entry.second.data == b && b->buffer.get() == nullptr) {
                ...
                if (list_hints == 0) {
                    ...
                    // otherwise, we can allocate up front, skipping the first
                    // pass.
                    const size_t list_size_multiplier =
                        (entry.second.data->isList ? entry.second.list_size_hint
                                                   : 1);
...
std::shared_ptr<PlyData> PlyFile::PlyFileImpl::request_properties_from_element(
    ...
    // used by each individual property.
    ...
    if (elementIndex >= 0) {
        ...
        for (auto key : propertyKeys) {
            const int64_t propertyIndex =
                find_property(key, element.properties);
...
void PlyFile::PlyFileImpl::parse_data(std::istream& is, bool firstPass) {
    ...
    auto read_list_binary = [this](const Type& t, void* dst, size_t& destOffset,
                                   const size_t& stride, std::istream& _is) {
        destOffset += stride;
    ...
    if (isBinary) {
        read = [this, &listSize, &dummyCount, &read_list_binary](
            ...
            if (!p.isList) {
                read_property_binary(p.propertyType, f.prop_stride,
                                     dest + destOffset, destOffset, _is);
            } else {
                read_list_binary(p.listType, &listSize, dummyCount,
                                 f.list_stride, _is);  // the list size
        ...
        skip = [this, &listSize, &dummyCount, &read_list_binary](
            ...
            read_list_binary(
                p.listType, &listSize, dummyCount, f.list_stride,
                _is);  // the list size (does not count for memory alloc)
            return read_property_binary(p.propertyType,
                                        f.prop_stride * listSize, scratch,
                                        dummyCount, _is);
        };
    } else {
        read = [this, &listSize, &dummyCount](
                   ...
                   size_t& destOffset, std::istream& _is) {
            if (!p.isList) {
                read_property_ascii(p.propertyType, f.prop_stride,
                                    dest + destOffset, destOffset, _is);
            } else {
                read_property_ascii(p.listType, f.list_stride, &listSize,
                                    dummyCount, _is);  // the list size
